...
// 外部ファイルの読み込み
function preload() {
    cmfTable = loadTable("/simulations/2024_cellophane-color/data/cmf.csv", "csv", "header") // 等色関数のデータ
    osTable = loadTable("/simulations/2024_cellophane-color/data/os_PC.csv", "csv", "header") // 偏光板を一枚通したときの波長毎の強度分布
    dTable = loadTable("/simulations/2024_cellophane-color/data/data_d_100.csv", "csv", "header")//光路差の分散特性(380nmで100に規格化)
...
function createCellophane(n, rAfter, a,angle_1) {
    ...
    fill(157, 204, 224, 10) //2024.6.14 透明度を50から20へ変更
    for (let i = 0; i < n; i++) {
        push()
